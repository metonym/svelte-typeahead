...
| extract               | `(TItem) => any`                                                           | `(item) => item`  | Target an item key if `data` is an object array                                                                                       |
| disable               | `(TItem) => boolean`                                                       | `(item) => false` | Pass in a function to disable items. They can be displayed in the results but will not be selectable.                                 |
| filter                | `(TItem) => boolean`                                                       | `(item) => false` | Pass in a function to filter items. They will be hidden and are not displayed in the results.                                         |
| autoselect            | `boolean`                                                                  | `true`            | Automatically select the first (top) result                                                                                           |
